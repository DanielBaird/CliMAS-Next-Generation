...
All of Australia is projected to experience warming in the future.  Currently, the mean annual temperature across {{ rg_short_name }} is {{ baseline_t_mean }}&deg;C, experiencing a range of averages between {{t_current_min }}&deg;C and {{ t_current_max }}&deg;C.

By {{rpt_year}}, temperature is projected to

[[hi_{{rpt_year}}_50th_t_mean > baseline_t_mean]] increase by {{hi_{{rpt_year}}_50th_t_mean - baseline_t_mean}} &deg;C to {{hi_{{rpt_year}}_50th_t_mean}}&deg;C.

[[hi_{{rpt_year}}_50th_t_mean < baseline_t_mean]]
decrease by 
$$t_high_{{rpt_year}}_fiftieth_d_down
&deg;C to 
$$hi_{{rpt_year}}_50th_t_mean
&deg;C, however 
...
[[      high_delta_up_{{rpt_year}}   == 0
    and high_delta_down_{{rpt_year}} == 0
]]
...
[[  high_total_added_{{rpt_year}} == high_total_lost_{{rpt_year}}
]]
...
unsuitable for $$high_total_lost_{{rpt_year}} species and suitable for $$high_total_added {{rpt_year}} species.

[[      high_delta_up_{{rpt_year}}    > 0
    and high_total_lost_{{rpt_year}} == 0
]]
...
likely to lose suitable climate space for of some species currently found in the region.  The climate is projected to become unsuitable for $$high_total_lost_{{rpt_year}} species and suitable for $$high_total_added_{{rpt_year}} species species from outside the region.

[[      high_total_added_{{rpt_year}} == 0
    and high_total_lost_{{rpt_year}}  > 0
]]
...
[[    high_total_lost_{{rpt_year}} > high_total_added_{{rpt_year}}
    and high_total_added_{{rpt_year}} > 0
]]



$$rg_long_name currently has climate suitable for $$high_total_total_current
species. By  {{rpt_year}} this number is projected to decrease to $$high_total_total_{{rpt_year}} species. While there is an overall decrease in biodiversity, the climate space of the region is likely to become suitable for some species currently not found in the region.  The climate is projected to become unsuitable for $$high_total_lost_{{rpt_year}} species and suitable for $$high_total_added_{{rpt_year}} species from outside the region.
...
<table>
    <tr>
        ...
        <td>
            ...
            (<span class="gained">+$$high_total_added_{{rpt_year}}</span>
            <span class="lost">-$$high_total_lost_{{rpt_year}}</span>)
        </td>
    </tr>
</table>
